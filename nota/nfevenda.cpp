    ...
    : QObject{parent}, m_nfe{new CppNFe}
{
    ...
    connect(m_nfe, &CppNFe::errorOccurred, this, &NFeVenda::errorOccurred);
...
void NFeVenda::ide()
{
    ...
    m_nfe->notafiscal->NFe->obj->infNFe->ide->set_natOp("VENDA DE MERCADORIA ADQUIRIDA");
    ...
    m_nfe->notafiscal->NFe->obj->infNFe->ide->set_tpImp(TpImp::Retrato); //TpImp::Retrato
...
void NFeVenda::retirada()
{
}

void NFeVenda::entrega()
{
}

void NFeVenda::autXML()
{
...
void NFeVenda::det(int quantProdutos)
{
...
void NFeVenda::det_produto(const int &i)
{
...
void NFeVenda::det_imposto(const int &i)
{
    QList<QVariant> produto = listaProdutos[i];
    //QString id            = produto[0].toString();
    //float quantVendida  = produto[1].toFloat();
...
void NFeVenda::det_impostoDevol()
{
    /*
...
void NFeVenda::transp_transporta()
{
    /*
    //Grupo Transportador
...
void NFeVenda::transp_retTransp()
{
    /*
...
void NFeVenda::transp_veicTransp()
{
    /*
...
void NFeVenda::transp_vol()
{
    ...
    // m_nfe->notafiscal->NFe->obj->infNFe->transp->vol->obj->set_pesoL(0);
    // m_nfe->notafiscal->NFe->obj->infNFe->transp->vol->obj->set_pesoB(0);
...
void NFeVenda::onReqStatusServico()
{
    QString _status;
...
void NFeVenda::onReqGerarEnviar()
{
    nfe();
...
void NFeVenda::onWSChange(const WebServicesNF &webServicesNF)
{
    QString _ws;
...
const CppNFe *NFeVenda::getCppNFe()
{
    return this->m_nfe;
}


void NFeVenda::setProdutosVendidos(QList<QList<QVariant>> produtosVendidos, bool emitirTodos){
    if (!db.open()) {
...
void NFeVenda::setPagamentoValores(QString formaPag, float desconto,float recebido, float troco, float taxa){
    if(formaPag == "Prazo"){
        indPagNf = "1";
        tPagNf = "15";
...
void NFeVenda::aplicarAcrescimoProporcional(float taxaPercentual)
{
...
void NFeVenda::aplicarDescontoTotal(float descontoTotal) {
...
float NFeVenda::corrigirDescontoParaAplicacaoPosTaxa(float descontoDesejado, float taxaPercentual) {
    float fatorTaxa = 1.0 + (taxaPercentual / 100.0);
    return descontoDesejado * fatorTaxa;
}
float NFeVenda::corrigirTaxa(float taxaAntiga, float desconto){
